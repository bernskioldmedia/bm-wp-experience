	...
	protected static function test(): array {
		$result = [
			...
			'description' => sprintf( '<p>%s</p>',
				__( 'This website is covered by a maintenance plan. That means it is getting the latest security updates and monitoring to keep it safe and secure.',
					'bm-wp-experience' ) ),
		];
		...
		if ( ! Updates::is_on_maintenance_plan() ) {
			...
			$result['description'] = sprintf( '<p>%s</p>',
				__( 'Not having the site on a maintenance plan means that you need to take extra care updating WordPress, plugins and themes to keep it up to date with the latest security fixes. Contact us at <a href="mailto:support@bernskioldmedia.com">support@bernskioldmedia.com</a> to find out more.',
					'bm-wp-experience' ) );
		}
